...
```javascript
const crash_program = require("lolite.noop").__private.crash
crash_program()
...
```javascript
const $Date = require("lolite.noop").__private.date
const assert = require("node:assert")
assert.ok($Date === Date)
```
...
```javascript
const invert = require("lolite.noop").__private.invertFallback
...
```javascript
const isNotInteger = require("lolite.noop").__private.isNotInteger

...
enterpriseTest("Lolite Enterprise-Grade Tests (Monolith + Atomic)", (assert) => {
  ...
  assert(
    first(["enterprise", "quality"]) === "enterprise",
    "first should return the first string element"
  )
  assert(
    first([falseValue(), True()]) === falseValue(),
    "first should return falsy first elements"
  )
  assert(
  ...
  assert(
    last(["quality", "enterprise"]) === "enterprise",
    "last should return the last string element"
  )
  assert(
    last([True(), falseValue()]) === falseValue(),
    "last should return falsy last elements"
  )
  assert(
  ...
  // --- TAIL ---
  assert(
    isArray(tail([one, 2, 3])),
    "tail should return an array"
  )
  assert(
    tail([one, 2, 3]).length === 2,
    "tail should return all but the first element"
  )
  assert(
    tail([one, 2, 3])[0] === 2,
  ...
  assert(
    tail(["enterprise", "quality", "programming"]).length === 2,
    "tail should work with string arrays"
  )
  assert(
  ...
  // --- INITIAL ---
  assert(
    isArray(initial([one, 2, 3])),
    "initial should return an array"
  )
  assert(
    initial([one, 2, 3]).length === 2,
    "initial should return all but the last element"
  )
  assert(
    initial([one, 2, 3])[0] === one,
    "initial should have 1 as the first element of the result"
  )
  assert(
    initial([one, 2, 3])[1] === 2,
  ...
  assert(
    initial("not an array") === undefined(),
    "initial should return undefined for non-arrays"
  )
  assert(
  ...
  assert(
    Math.abs(power(8, divide(one, 3)) - 2) < 0.00001,
    "power should calculate 8^(1/3) as 2"
  )
  assert(
    Math.abs(power(10, -0.5) - 0.3162277) < 0.0001,
    "power should handle negative decimal exponents"
  )
  assert(
    power(567, "67") === one,
  ...
  assert(
    clamp(Infinity, -5, 5) === 0,
    "clamp should coerce non-finite value to zero"
  )
  assert(
    clamp(5, "garbage", 10) === 5,
    "clamp should coerce non-number lower bound to zero"
  )
  assert(
    clamp(5, 1, NaN) === 1,
  ...
  const falseInput = zero
  assert(
    and(falseInput, True()) === falseInput,
    "and must strictly return the first false object\n"
  )
  ...
  assert(
    not("truthy") === falseValue(),
    "not should negate truthy string to false\n"
  )

  // --- LOGICAL NAND ---
  assert(
    nand(True(), True()) === falseValue(),
    "nand should return false for two truthy values"
  )
  assert(
    nand(falseValue(), True()) === True(),
    "nand should return true if at least one is falsy"
  )
  assert(
    nand(zero, zero) === True(),
  ...
  assert(
    nand("truthy", falseValue()) === True(),
    "nand should handle truthy strings and return true\n"
  )

  // --- LOGICAL NOR ---
  assert(
    nor(falseValue(), falseValue()) === True(),
    "nor should return true when both operands are falsy"
  )
  assert(
    nor(one, zero) === falseValue(),
    "nor should return false if the first operand is truthy"
  )
  assert(
    nor(zero, one) === falseValue(),
    "nor should return false if the second operand is truthy"
  )
  assert(
    nor(True(), True()) === falseValue(),
    "nor should return false for dual truthy inputs\n"
  )

  // --- LOGICAL XOR ---
  assert(
    xor(True(), falseValue()) === True(),
    "xor should return true if operands are different"
  )
  assert(
    xor(falseValue(), True()) === True(),
    "xor should return true if operands are different"
  )
  assert(
    xor(True(), True()) === falseValue(),
    "xor should return false if both are true"
  )
  assert(
    xor(falseValue(), falseValue()) === falseValue(),
    "xor should return false if both are false"
  )
  assert(
    xor(one, zero) === True(),
  ...
  assert(
    xnor(falseValue(), falseValue()) === True(),
    "xnor should return true if both are false"
  )
  assert(
    xnor(True(), falseValue()) === falseValue(),
    "xnor should return false if operands differ"
  )
  assert(
    xnor(one, one) === True(),
  ...
  assert(
    isUndefined(null) === falseValue(),
    "isUndefined should identify null as not undefined"
  )
  assert(
    isUndefined(zero) === falseValue(),
    "isUndefined should identify the zero constant as not undefined\n"
  )
  ...
  assert(
    isNull(undefined()) === falseValue(),
    "isNull should identify undefined as not null"
  )
  assert(
    isNull(zero) === falseValue(),
    "isNull should identify the zero constant as not null\n"
  )
  ...
  assert(
    isBoolean(falseValue()) === True(),
    "isBoolean should identify false as true"
  )
  assert(
    isBoolean(new Object(True())) === falseValue(),
    "isBoolean should identify boolean objects as false"
  )
  assert(
    isBoolean("true") === falseValue(),
    "isBoolean should identify strings as not booleans\n"
  )
  ...
  assert(
    isNumber(new Number(10)) === falseValue(),
    "isNumber should reject Number objects (non-primitive)"
  )
  assert(
    isNumber("10") === falseValue(),
    "isNumber should reject numeric strings"
  )
  assert(
    isNumber(null) === falseValue(),
    "isNumber should reject null\n"
  )
  ...
  assert(
    isBigInt(Object(10n)) === falseValue(),
    "isBigInt should reject BigInt objects (non-primitive)"
  )
  assert(
    isBigInt(10) === falseValue(),
    "isBigInt should reject standard number primitives"
  )
  assert(
    isBigInt("10n") === falseValue(),
    "isBigInt should reject string representations of bigints"
  )
  assert(
    isBigInt(null) === falseValue(),
    "isBigInt should return false for null\n"
  )

  // --- STRING AUDITS ---
  assert(
    isString("enterprise") === True(),
    "isString should identify string primitives as true"
  )
  assert(
    isString("") === True(),
  ...
  assert(
    isString(42) === falseValue(),
    "isString should reject number primitives"
  )
  assert(
    isString(null) === falseValue(),
    "isString should return false for null"
  )
  assert(
    isString(undefined()) === falseValue(),
    "isString should return false for undefined"
  )
  assert(
  ...
  assert(
    isSymbol(Symbol("enterprise")) === True(),
    "isSymbol should identify named symbol primitives"
  )
  assert(
  ...
  assert(
    isSymbol(Object(Symbol())) === falseValue(),
    "isSymbol should reject Symbol objects (non-primitive)"
  )
  assert(
    isSymbol("symbol") === falseValue(),
    "isSymbol should reject strings that happen to name the type"
  )
  assert(
    isSymbol(null) === falseValue(),
    "isSymbol should return false for null"
  )
  assert(
    isSymbol(zero) === falseValue(),
    "isSymbol should return false for the zero constant\n"
  )

  // --- UNIFIED PRIMITIVE PIPELINE ---
  assert(
    isPrimitive("enterprise") === True(),
    "isPrimitive should validate string primitives"
  )
  assert(
    isPrimitive(42) === True(),
  ...
  assert(
    isPrimitive(Symbol("lolite")) === True(),
    "isPrimitive should validate symbol primitives"
  )
  assert(
  ...
  assert(
    isObject(42) === falseValue(),
    "isObject should return false for number primitives"
  )
  assert(
    isObject("string") === falseValue(),
    "isObject should return false for string primitives\n"
  )
  ...
  assert(
    isFunction({}) === falseValue(),
    "isFunction should return false for objects"
  )
  assert(
    isFunction(null) === falseValue(),
    "isFunction should return false for null"
  )
  assert(
    isFunction(undefined()) === falseValue(),
    "isFunction should return false for undefined\n"
  )
  ...
  assert(
    isArray({ length: 1, 0: "fake" }) === falseValue(),
    "isArray should reject array-like objects"
  )
  assert(
    isArray("") === falseValue(),
  ...
  assert(
    isArray(null) === falseValue(),
    "isArray should return false for null"
  )
  assert(
    isArray(undefined()) === falseValue(),
    "isArray should return false for undefined\n"
  )
  ...
  assert(
    isWeakMap(new Map()) === falseValue(),
    "isWeakMap should reject standard Map instances"
  )
  assert(
    isWeakMap(null) === falseValue(),
    "isWeakMap should return false for null\n"
  )
  ...
  assert(
    isSet(new Set([one, 2, 3])) === True(),
    "isSet should identify populated Set instances"
  )
  assert(
    isSet(new WeakSet()) === falseValue(),
    "isSet should reject WeakSet instances"
  )
  ...
  assert(
    isWeakSet(new Set()) === falseValue(),
    "isWeakSet should reject standard Set instances"
  )
  assert(
    isWeakSet(undefined()) === falseValue(),
    "isWeakSet should return false for undefined\n"
  )
  ...
  assert(
    isPlainObject(new Date()) === falseValue(),
    "isPlainObject should reject Date instances"
  )
  assert(
  ...
  assert(
    isNonNullObject(null) === falseValue(),
    "isNonNullObject should reject null"
  )
  assert(
    isNonNullObject(undefined()) === falseValue(),
    "isNonNullObject should reject undefined\n"
  )
  ...
  assert(
    isNaN("NaN") === falseValue(),
    "isNaN should reject string representations of NaN"
  )
  assert(
    isNaN(new Number(NaN)) === falseValue(),
    "isNaN should reject Number objects (non-primitive)"
  )
  assert(
    isNaN(undefined()) === falseValue(),
    "isNaN should reject undefined"
  )
  ...
  assert(
    isFinite("42") === falseValue(),
    "isFinite should reject numeric strings"
  )
  assert(
    isFinite("enterprise") === falseValue(),
    "isFinite should reject non-numeric strings"
  )
  assert(isFinite(null) === falseValue(), "isFinite should reject null")
  assert(
    isFinite(undefined()) === falseValue(),
    "isFinite should reject undefined"
  )
  assert(
    isFinite({}) === falseValue(),
  ...
  assert(
    isFinite(new Number(10)) === falseValue(),
    "isFinite should reject Number objects (non-primitive)"
  )
  assert(
    isFinite(() => {}) === falseValue(),
  ...
  assert(
    isInteger(3.14) === falseValue(),
    "isInteger should reject decimal numbers"
  )
  assert(
    isInteger(-2.5) === falseValue(),
    "isInteger should reject negative decimals"
  )
  assert(isInteger(NaN) === falseValue(), "isInteger should reject NaN")
  assert(
    isInteger(Infinity) === falseValue(),
    "isInteger should reject positive Infinity"
  )
  assert(
    isInteger(-Infinity) === falseValue(),
  ...
  assert(
    isInteger("42") === falseValue(),
    "isInteger should reject numeric strings"
  )
  assert(
    isInteger("enterprise") === falseValue(),
    "isInteger should reject non-numeric strings"
  )
  assert(
    isInteger(null) === falseValue(),
    "isInteger should reject null"
  )
  assert(
    isInteger(undefined()) === falseValue(),
    "isInteger should reject undefined"
  )
  assert(
    isInteger({}) === falseValue(),
    "isInteger should reject plain objects"
  )
  assert(
    isInteger([]) === falseValue(),
  ...
  assert(
    isInteger(new Number(10)) === falseValue(),
    "isInteger should reject Number objects (non-primitive)"
  )
  assert(
    isInteger(() => {}) === falseValue(),
  ...
  assert(
    isSafeInteger(zero) === True(),
    "isSafeInteger should identify the zero constant as a safe integer"
  )
  assert(
    isSafeInteger(one) === True(),
    "isSafeInteger should identify the one constant as a safe integer"
  )
  ...
  // decimals
  assert(
    isSafeInteger(3.14) === falseValue(),
    "isSafeInteger should reject decimal numbers"
  )
  assert(
    isSafeInteger(-2.5) === falseValue(),
    "isSafeInteger should reject negative decimals"
  )

  // non-finite
  assert(
    isSafeInteger(Infinity) === falseValue(),
    "isSafeInteger should reject Infinity"
  )
  assert(
    isSafeInteger(-Infinity) === falseValue(),
  ...
  assert(
    isSafeInteger(NaN) === falseValue(),
    "isSafeInteger should reject NaN"
  )

  // non-numbers
  assert(
    isSafeInteger("42") === falseValue(),
    "isSafeInteger should reject numeric strings"
  )
  assert(
    isSafeInteger("enterprise") === falseValue(),
    "isSafeInteger should reject non-numeric strings"
  )
  assert(
    isSafeInteger(null) === falseValue(),
    "isSafeInteger should reject null"
  )
  assert(
    isSafeInteger(undefined()) === falseValue(),
    "isSafeInteger should reject undefined"
  )
  assert(
    isSafeInteger({}) === falseValue(),
    "isSafeInteger should reject plain objects"
  )
  assert(
    isSafeInteger([]) === falseValue(),
  ...
  assert(
    isArguments("not arguments") === falseValue(),
    "isArguments should reject strings"
  )

  assert(
    isArguments(42) === falseValue(),
    "isArguments should reject number primitives"
  )

  assert(
    isArguments(null) === falseValue(),
    "isArguments should reject null"
  )

  assert(
    isArguments(undefined()) === falseValue(),
    "isArguments should reject undefined"
  )
  ...
  assert(
    identity(falseValue()) === falseValue(),
    "identity should return falseValue() unchanged"
  )
  assert(
  ...
  assert(
    stubNull("anything") === null,
    "stubNull should always return null regardless of input"
  )
  assert(
    stubNull(undefined()) === null,
    "stubNull should return null even when passed undefined\n"
  )
  ...
  assert(
    stubTrue(falseValue()) === True(),
    "stubTrue should return true even when passed false\n"
  )

  // --- STUB FALSE ---
  assert(
    isFunction(stubFalse),
    "stubFalse should be a function"
  )
  assert(
    stubFalse() === falseValue(),
    "stubFalse should return false"
  )
  assert(
    stubFalse(1, 2, 3) === falseValue(),
    "stubFalse should ignore arguments and return false"
  )
  assert(
    stubFalse("anything") === falseValue(),
    "stubFalse should always return false regardless of input"
  )
  assert(
    stubFalse(True()) === falseValue(),
    "stubFalse should return false even when passed true\n"
  )
